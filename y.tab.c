...
{
  "$end", "error", "$undefined", "exit_command", "digit", "print",
  "$accept", "s", "numero", YY_NULLPTR
};
#endif
...
yyreduce:
  ...
  case 3:
#line 30 "roman.y"
                            {strcpy(r,calculateRoman(r,(yyvsp[-1].A).length));printf("The roman numeral is: %s\n",r);exit(0);}
#line 1316 "y.tab.c"
    break;

  case 4:
#line 33 "roman.y"
                            {temp = sheelam((yyvsp[0].num));strncat(r,&temp,1);(yyval.A).length=(yyvsp[-1].A).length+1;}
#line 1322 "y.tab.c"
    break;

  case 5:
#line 34 "roman.y"
                            {temp = sheelam((yyvsp[0].num));strncat(r,&temp,1); (yyval.A).length = 1;}
#line 1328 "y.tab.c"
    break;
